...
Rcpp::List MPC_qr(SEXP & nn, SEXP & data, SEXP & theta_boot, SEXP & data_boot, SEXP & alpha, SEXP & M_samp, SEXP & B_resamp) { 
...
Rcpp::List MPC_linreg(SEXP & nn, SEXP & data, SEXP & theta_boot, SEXP & data_boot, SEXP & alpha, SEXP & M_samp, SEXP & B_resamp) { 
...
for (int i=0; i<B; i++) {
	...
	for(int j=0; j<(M+100); j++) {
		...
		loglikdiff = 0.0;
		for(int k=0; k<n; k++){
			loglikdiff = loglikdiff -w * pow(databoot(k,i)-theta0new*ddata(k,1)-theta1old*ddata(k,2)-theta2old*ddata(k,3)-theta3old*ddata(k,4),2) + w*pow(databoot(k,i)-theta0old*ddata(k,1)-theta1old*ddata(k,2)-theta2old*ddata(k,3)-theta3old*ddata(k,4),2); 
		}
		r = Rcpp::dnorm(theta0new, theta0old[0],.5)/Rcpp::dnorm(theta0old,theta0new[0],.5);
		loglikdiff[0] = loglikdiff[0] + log(r(0));
		loglikdiff[0] = fmin(std::exp(loglikdiff[0]), 1.0);
		...
		loglikdiff = 0.0;
		for(int k=0; k<n; k++){
			loglikdiff = loglikdiff -w * pow(databoot(k,i)-theta0old*ddata(k,1)-theta1new*ddata(k,2)-theta2old*ddata(k,3)-theta3old*ddata(k,4),2) + w*pow(databoot(k,i)-theta0old*ddata(k,1)-theta1old*ddata(k,2)-theta2old*ddata(k,3)-theta3old*ddata(k,4),2); 
		}
		r = Rcpp::dnorm(theta1new, theta1old[0],.5) / Rcpp::dnorm(theta1old,theta1new[0],.5);
		loglikdiff[0] = loglikdiff[0] + log(r(0));
		loglikdiff[0] = fmin(std::exp(loglikdiff[0]), 1.0);
		...
		loglikdiff = 0.0;
		for(int k=0; k<n; k++){
			loglikdiff = loglikdiff -w * pow(databoot(k,i)-theta0old*ddata(k,1)-theta1old*ddata(k,2)-theta2new*ddata(k,3)-theta3old*ddata(k,4),2) + w*pow(databoot(k,i)-theta0old*ddata(k,1)-theta1old*ddata(k,2)-theta2old*ddata(k,3)-theta3old*ddata(k,4),2); 
		}
		...
		loglikdiff = 0.0;
		for(int k=0; k<n; k++){
			loglikdiff = loglikdiff -w * pow(databoot(k,i)-theta0old*ddata(k,1)-theta1old*ddata(k,2)-theta2old*ddata(k,3)-theta3new*ddata(k,4),2) + w*pow(databoot(k,i)-theta0old*ddata(k,1)-theta1old*ddata(k,2)-theta2old*ddata(k,3)-theta3old*ddata(k,4),2); 
		}
	...
	l3 = sort3(0.025*M);
	u3 = sort3(0.975*M);
	if ( (l1 < bootmean1(0)) && (u1 > bootmean1(0)) &&  (l0 < bootmean0(0)) && (u0 > bootmean0(0)) &&  (l2 < bootmean2(0)) && (u2 > bootmean2(0)) && (l3 < bootmean3(0)) && (u3 > bootmean3(0)) ){
		cover(i) = 1.0;
	} else {cover(i) = 0.0;}
}
sumcover = 0.0;
...
if(((abs(diff)<= eps)&&(diff>=0)) || t>16) {
	...
	t = t+1;
	w = fmax(w + (pow(1+t,-0.51)*diff),0.01);
}
}
...
theta3old[0] = bootmean3(0);
	for(int j=0; j<(2*M+1000); j++) {
		...
		loglikdiff = 0.0;
		for(int k=0; k<n; k++){
			loglikdiff = loglikdiff -w * pow(ddata(k,0)-theta0new*ddata(k,1)-theta1old*ddata(k,2)-theta2old*ddata(k,3)-theta3old*ddata(k,4),2) + w*pow(ddata(k,0)-theta0old*ddata(k,1)-theta1old*ddata(k,2)-theta2old*ddata(k,3)-theta3old*ddata(k,4),2); 
		}
		r = Rcpp::dnorm(theta0new, theta0old[0],.5)/Rcpp::dnorm(theta0old,theta0new[0],.5);
		loglikdiff[0] = loglikdiff[0] + log(r(0));
		loglikdiff[0] = fmin(std::exp(loglikdiff[0]), 1.0);
		...
		loglikdiff = 0.0;
		for(int k=0; k<n; k++){
			loglikdiff = loglikdiff -w * pow(ddata(k,0)-theta0old*ddata(k,1)-theta1new*ddata(k,2)-theta2old*ddata(k,3)-theta3old*ddata(k,4),2) + w*pow(ddata(k,0)-theta0old*ddata(k,1)-theta1old*ddata(k,2)-theta2old*ddata(k,3)-theta3old*ddata(k,4),2); 
		}
		r = Rcpp::dnorm(theta1new, theta1old[0],.5) / Rcpp::dnorm(theta1old,theta1new[0],.5);
		loglikdiff[0] = loglikdiff[0] + log(r(0));
		loglikdiff[0] = fmin(std::exp(loglikdiff[0]), 1.0);
		...
		loglikdiff = 0.0;
		for(int k=0; k<n; k++){
			loglikdiff = loglikdiff -w * pow(ddata(k,0)-theta0old*ddata(k,1)-theta1old*ddata(k,2)-theta2new*ddata(k,3)-theta3old*ddata(k,4),2) + w*pow(ddata(k,0)-theta0old*ddata(k,1)-theta1old*ddata(k,2)-theta2old*ddata(k,3)-theta3old*ddata(k,4),2); 
		}
		r = Rcpp::dnorm(theta2new, theta2old[0],.5) / Rcpp::dnorm(theta2old,theta2new[0],.5);
		loglikdiff[0] = loglikdiff[0] + log(r(0));
		loglikdiff[0] = fmin(std::exp(loglikdiff[0]), 1.0);
		...
		loglikdiff = 0.0;
		for(int k=0; k<n; k++){
			loglikdiff = loglikdiff -w * pow(ddata(k,0)-theta0old*ddata(k,1)-theta1old*ddata(k,2)-theta2old*ddata(k,3)-theta3new*ddata(k,4),2) + w*pow(ddata(k,0)-theta0old*ddata(k,1)-theta1old*ddata(k,2)-theta2old*ddata(k,3)-theta3old*ddata(k,4),2); 
		}
		r = Rcpp::dnorm(theta3new, theta3old[0],.5) / Rcpp::dnorm(theta3old,theta3new[0],.5);
		loglikdiff[0] = loglikdiff[0] + log(r(0));
		loglikdiff[0] = fmin(std::exp(loglikdiff[0]), 1.0);
...
Rcpp::List MPC_varmix(SEXP & nn, SEXP & data, SEXP & data_boot, SEXP & alpha, SEXP & B_resamp) { 
RNGScope scp;
List result;
