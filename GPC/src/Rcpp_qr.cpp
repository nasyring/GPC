...
NumericVector data_ = Rcpp::as<NumericVector>(data);
NumericVector thetaboot_ = Rcpp::as<NumericVector>(theta_boot);
		...
		for(int k=0; k<n; k++){
			...
			if(databoot(k,2*i+1)==(-1.0)){
				if(theta0new(0) + theta1old(0)*databoot(k,2*i) <= 0.0){
					...
					lossnew(0) = 0.5 * pow((1- (theta0new(0) + theta1old(0)*databoot(k,2*i))),2.0);	
				}else {
					lossnew(0) = 0.0;
				}				
			}else {
		...
		for(int k=0; k<n; k++){
			...
			if(databoot(k,2*i+1)==(-1.0)){
				if(theta0old(0) + theta1new(0)*databoot(k,2*i) <= 0.0){
					...
					lossnew(0) = 0.5 * pow((1- (theta0old(0) + theta1new(0)*databoot(k,2*i))),2.0);	
				}else {
					lossnew(0) = 0.0;
				}				
			}else {
...
struct GPC_class_mcmc_parallel : public Worker {
   ...
   // initialize with source and destination
   GPC_class_mcmc_parallel(const NumericVector nn,	const NumericMatrix data, const NumericMatrix thetaboot,
	const NumericVector bootmean0, const NumericVector bootmean1, const NumericVector propvar, const NumericMatrix databoot,
	const NumericVector alpha, const NumericVector M_samp, const NumericVector B_resamp,
	const NumericVector w, NumericVector cover) 
			: nn(nn), data(data), thetaboot(thetaboot), bootmean0(bootmean0), bootmean1(bootmean1), propvar(propvar), databoot(databoot), alpha(alpha), M_samp(M_samp), B_resamp(B_resamp), w(w), cover(cover) {}   
...
void operator()(std::size_t begin, std::size_t end) {
		for (std::size_t i = begin; i < end; i++) {
			cover[i] = GibbsMCMCclass(nn, data, thetaboot, bootmean0, bootmean1, propvar, databoot, alpha, M_samp, w, i);	
		}
	}
};

// [[Rcpp::export]]
NumericVector rcpp_parallel_class(NumericVector nn, NumericMatrix data, NumericMatrix thetaboot, NumericVector bootmean0,
	NumericVector bootmean1, NumericVector propvar, NumericMatrix databoot, NumericVector alpha, NumericVector M_samp, NumericVector B_resamp,
	NumericVector w) {
   ...
   // create the worker
   GPC_class_mcmc_parallel gpcWorker(nn, data, thetaboot, bootmean0, bootmean1, propvar, databoot, alpha, M_samp, B_resamp, w, cover);
